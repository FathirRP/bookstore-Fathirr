<x-layouts.admin title="Edit Kategori" header="Edit Kategori">
    <div class="max-w-lg">
        ...
        <div class="bg-white rounded-2xl border border-slate-200/60 overflow-hidden">
            <div class="px-6 py-5 border-b border-slate-100 bg-slate-50/50">
                <h3 class="text-base font-bold text-slate-800">Edit Kategori</h3>
                <p class="text-xs text-slate-400 mt-0.5">Perbarui nama kategori</p>
            </div>
            <form method="POST" action="{{ route('admin.categories.update', $category) }}" class="p-6">
                @csrf
                @method('PUT')
                <x-input label="Nama Kategori" name="name" :value="$category->name" required />
